...
public class ChilliPepper extends Plant {
    ...
    public ChilliPepper(int x, int y, int column, int row) {
        super(x, y, 100, 100, "/assets/gif/chillipepper.gif", 4, column, row);
    ...
    public void buildImage(GridPane lawn) {
        ...
        for (int idx = 0; idx < 9; idx++) {
            fires[idx] = new ImageView(new Image(Objects.requireNonNull(getClass().getResource("/assets/gif/chillipepperFire.gif")).toString(), 100, 100, false, false));
    ...
    public void attacking(Pane pane) {
        new Thread(() -> {
            ...
            Media blast = new Media(Objects.requireNonNull(getClass().getResource("/assets/sounds/chillipepper.wav")).toString());
