...
public class Pea extends Entity {
    ...
    public Pea(int x, int y, int plantPosition, int lane) {
        ...
        this.path = Objects.requireNonNull(getClass().getResource("/assets/images/pea.png")).toString();
            ...
            for (Zombie zombie : GameController.allZombies) {
                if (zombie.getLane() == lane && !bombed) {
                    if (Math.abs(zombie.getX() - getX()) <= 3 && !bombed) {
                        ...
                        peaAnimation.stop();
                        Media splat = new Media(Objects.requireNonNull(getClass().getResource("/assets/sounds/splat3.wav")).toString());
