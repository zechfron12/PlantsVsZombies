...
public abstract class Zombie extends Entity implements Attackable {
        ...
        Platform.runLater(() -> {
            if (health <= 0) {
                ...
                for (Object zombie : GameController.allZombies) {
                    if (this == zombie) {
                        Media yuckSound = new Media(Objects.requireNonNull(getClass().getResource("/assets/sounds/yuck.wav")).toString());
            ...
            if (health <= 7) {
                getImage().setImage(new Image(Objects.requireNonNull(getClass().getResource("/assets/gif/defaultzombie.gif")).toString(), 65, 115, false, false));
                width = 65;
                height = 115;
            }
        });
    ...
    public void burntZombie() {
        getImage().setImage(new Image(Objects.requireNonNull(getClass().getResource("/assets/gif/burntZombie.gif")).toString(), 65, 115, false, false));
        health = 0;
        dx = 0;
        ++GameController.numKilledZombies;
        new Thread(() -> {
    ...
    public void ReachedHouse() {
        if (getImage().getX() <= 220) {
            Media brainzSound = new Media(Objects.requireNonNull(getClass().getResource("/assets/sounds/brainz.wav")).toString());
